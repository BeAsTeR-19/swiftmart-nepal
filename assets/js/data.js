...
const SM = {
  ...
  productCard(p) {
    ...
    return `<div class="product-card" data-id="${p.id}">
      ...
      <button class="pc-wish ${wishlisted ? 'active' : ''}" onclick="event.stopPropagation(); SM.toggleWishlist('${p.id}'); const isW = SM.isWishlisted('${p.id}'); this.classList.toggle('active', isW); this.innerHTML = '<svg width=\'18\' height=\'18\' viewBox=\'0 0 24 24\' fill=\'' + (isW ? '#ff0000' : 'none') + '\' stroke=\'' + (isW ? '#ff0000' : '#999') + '\' stroke-width=\'2\'><path d=\'M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z\'/></svg>'; if(window.location.search.includes('wishlist=true')) setTimeout(()=>location.reload(), 300);" aria-label="Wishlist">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="${wishlisted ? '#ff0000' : 'none'}" stroke="${wishlisted ? '#ff0000' : '#999'}" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
      </button>
      <div class="pc-body">
